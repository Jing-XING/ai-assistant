...
一个基于 Codex 的个人 AI 助理、任务看板、提醒系统和操作桥。页面默认按北京时间展示任务、时间块、番茄钟、归档报告和浮动聊天窗；本地 SQLite 负责静态存储。
...
- 总览、股票复盘、任务、归档、专注、设置六个主页面。
- Codex 不再占用侧边栏页面，而是作为全局浮动聊天窗使用。
- SQLite 存储任务、提醒、网页留言、Codex 处理事件和周报/月报。
- 股票复盘页按北京时间更新美股隔夜和 A 股收盘的结构化统计数据，并在页面内展示文字摘要、指数卡片、板块条形图和领涨领跌列表。
